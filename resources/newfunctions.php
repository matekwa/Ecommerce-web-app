...
function get_suggestions(){
	...
	while($row = fetch_array($query)){
		...
		<a href="single_product.php?id={$row['ID']}&c_id={$row['category_ID']}"> <img src="../resources/uploads/{$row['product_image']}" alt="{$row['Title']}" class="img-fluid" title="{$row['Title']}" style = "width:170px;height:170px; border-radius:5px"></a>
			...
			<h3 style="font-family:serif">{$row['Title']}</h3>
			<sup>WAS</sup>
...
function get_bestselling_brands()
{
	...
	while ($row = fetch_array($query)) {
		...
		<div class="col-md-2 bestselling_product pt-md-5 pt-4">
			...
			<a href="brands.php?id={$brand_id}"><span class="border product-btn btn-span">{$product_name}</span></a>
		</div>
DELIMETER;
		echo $bestseller;
	}
 ...
 function get_slides()
 {
 	...
 	while ($row = fetch_array($query)) {
		...
		<div class="carousel-item" data-bs-interval="3000">
		  <a href=""><img src="{$slide}" class="d-block w-100" alt="{$row['slide_title']}"></a>
		</div>
...
if (isset($_POST['u'])){
   ...
   //$conn = mysqli_connect("localhost","swifftsh_ronald","MatekwaRonald37016568","swifftsh_swifftshop");
    ...
    if ($email == "" || $username == ""||$password == "" ||$county == ""|| $gender == "") {
        ...
        $sql = "INSERT INTO newregisteredusers(`Email`, `Username`,`user_photo`, `Password`, `Gender`, `County`, `validation_code`, `Active`, `IPAddress`, `Last_Login`) VALUES ('$email','$username','','$p_hash','$gender','$county','$validation_code','0','$ip',now()";
        $query = mysqli_query($conn,$sql);
		...
		if (isset($_SESSION['token']) && $_POST['token'] == $_SESSION['token']) {
			...
			////////////////////////////Mailing System/////////////////////////////////////////
				 /*ob_start();
                    ...
                    $message = '<!DOCTYPE html><html lang="eng"><head><meta charset="UTF-8"><title>Password Recovery</title></head><body style="margin:0px; font-family:Tahoma, Geneva, sans-serif;"><div style="padding:10px; background:#45ccb8; font-size:24px; color:#CCC;"><a href="http://www.swifftshop.com"><img src="http://www.swifftshop.com/resources/uploads/swifftshop.png" width="36" height="30" alt="swifftshop.com" style="border:none; float:left;"></a>Swifftshop Account Activation</div><div style="padding:24px; font-size:17px;">Click the link below to reset your password<br><br><a href="www.swifftshop.com/code.php?Email=$email&code=$code">Click here to Reset your Password!</a><br/>And use the code <b>$code</b> <br /></div></body></html>';
                    try {
                                //Server settings
                                $mail->SMTPDebug = SMTP::DEBUG_SERVER;                      //Enable verbose debug output
                                $mail->isSMTP();                                            //Send using SMTP
			 		...
			 		if ($_SESSION['token'] == $_POST['token2']) {
			 			if ($_POST['newPassword1'] === $_POST['newPassword2']) {
			 				...
			 				if ($query) {
			 					...
			 					set_message("<div class='alert alert-warning text-center'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Something went wrong :(</div>");
			 				}
			 			} else{
...
//////////////////////////////////////////////////Edit user details function from profile page////////////////////////////////////////////////////////////////
...
function edit_user_password(){
 ...
 function add_to_cart_directly(product_id){
        ...
        cart_product.send("add_to_cart="+product_id+"&product_size="+"Unselected");
   ...
   function buss_Info(){
   		...
   		if (shop_location.length == 0 || businessName.length == 0 || region.length == 0 || town.length == 0 ||  vat_registered.length == 0 || businessRegNumber.length == 0 || supplyCategory.length == 0) {
             ...
             document.getElementById("seller_message").innerHTML = '<div style="color:#fff;font-weight:bold;background:green">Processing...</div>';
            let ajax = myAjax("POST", "../resources/functions.php");
            ajax.onreadystatechange = function(){
                if (ajaxStatus(ajax) == true){
...
function bank_info() {
	...
	if (mpesaRegisteredName.length == 0 || modeOfPayment.length == 0 || mpesaPhoneNumber.length == 0 || accountName.length == 0 ||  accountNumber.length == 0 || bankName.length == 0) {
            ...
            let ajx = myAjax("POST", "../resources/functions.php");
            ajx.onreadystatechange = function(){
                if (ajaxStatus(ajx) == true){
                    if ((ajx.responseText.replace(/^\s+|\s+$/g, "")) == "done"){
                    	window.scrollTo(0,0);
                        document.getElementById("bank_account_info").innerHTML = "Your details are submitted,we`ll review them and get back to you as soon as possible.";
                    } else {
